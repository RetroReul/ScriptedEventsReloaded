...
public class BoolValue(bool underlyingValue) : LiteralValue<bool>(underlyingValue), IValueWithProperties
{
    [UsedImplicitly]
    ...
    public override string StringRep => UnderlyingValue.ToString().ToLowerInvariant();
    ...
    {
        ["not"] = new Prop<BoolValue>(b => !b.UnderlyingValue, "Inverted boolean value"),
        ["asNumber"] = new Prop<NumberValue>(b => b.UnderlyingValue ? 1m : 0m, "Converts boolean to number (1 for true, 0 for false)"),
        ["asString"] = new Prop<StaticTextValue>(b => b.UnderlyingValue.ToString().ToLowerInvariant(), "Converts boolean to string ('true' or 'false')"),
